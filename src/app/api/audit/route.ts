...
/* PSI with visible error messages */
const PSI_ENDPOINT = "https://www.googleapis.com/pagespeedonline/v5/runPagespeed";
type PSIResult = { score?: number; raw?: any; error?: string };
async function runPSI(target: string, strategy: "mobile" | "desktop", key?: string): Promise<PSIResult> {
  if (!key) return { error: "PSI_API_KEY missing" };
  try {
    ...
    if (!res.ok) {
      ...
      return { error: `PSI ${strategy} failed: ${msg}`, raw: json };
    }
    const score = json?.lighthouseResult?.categories?.performance?.score;
    return { score: typeof score === "number" ? Math.round(score * 100) : undefined, raw: json };
  } catch (e: any) {
    return { error: `PSI ${strategy} exception: ${e?.message || String(e)}` };
  }
}
function pickTopActionsFromRaw(raws: any[]): string[] {
  try {
    ...
    for (const r of raws) {
      ...
      for (const a of Object.values(audits) as any[]) {
        ...
        const scoredLow = typeof a?.score === "number" && a.score < 1;
        if ((isOpp || scoredLow) && a?.title) titles.add(a.title);
      }
    }
...
async function crawlSmall(origin: string, limit = 6) {
  ...
  for (const p of HIGH_SIGNAL_PATHS.slice(0, limit)) {
    ...
    if (res.ok) { const html = await res.text(); pages.push({ url: res.url, html, headers: res.headers }); }
  }
  return pages;
}
...
/* On-page checks + Entity Trust & Hygiene */
...
function evaluateHome(html: string, headers: Headers, url: string): Issue[] {
  ...
  issues.push({ id:"title_tag", status: tl>=15&&tl<=60? "pass" : (tl>=8&&tl<=70? "warn":"fail"), details:{ length:tl, title }, fix:"Craft unique, descriptive titles (≤60 chars).", page:url, category:"onpage_seo" });

  const md = /<meta[^>]+name=['"]description['"][^>]+content=['"]([^'"]*)['"][^>]*>/i.exec(html)?.[1] || "";
  const mdl = md.trim().length;
  issues.push({ id:"meta_description", status: mdl===0? "fail" : (mdl<70||mdl>160? "warn":"pass"), details:{ length:mdl }, fix:"Write ~150-char descriptions matching intent.", page:url, category:"onpage_seo" });
  ...
  const internal = links.filter(l=>{ try { const abs = new URL(l.href, url); return abs.origin===origin; } catch { return false; } });
  const texts = internal.map(l=>l.text.toLowerCase()).filter(Boolean);
  const uniqueAnchors = new Set(texts).size;
  const anchorDiversity = texts.length? uniqueAnchors/texts.length : 1;
  issues.push({ id:"internal_links", status: (internal.length>=5 && anchorDiversity>=0.4)? "pass" : (internal.length>=3? "warn":"fail"), details:{ internalCount:internal.length, anchorDiversity }, fix:"Add contextual internal links and diversify anchors.", page:url, category:"onpage_seo" });
...
function scoreCategories(issues: Issue[]) {
  ...
  for (const cat of categories) {
    const entries = Object.entries(weights[cat] || {});
...
async function auditOne(origin: string, options: any, PSI_KEY?: string) {
  ...
  // PSI calls with debug
  const m = await runPSI(origin, "mobile", PSI_KEY);
  const d = await runPSI(origin, "desktop", PSI_KEY);
  const mobileScore = m.score;
  const desktopScore = d.score;
  const topActions = pickTopActionsFromRaw([m.raw, d.raw].filter(Boolean));
  const pagespeedDebug = { mobileError: m.error || null, desktopError: d.error || null };
  ...
  return {
    ...
    pagespeedDebug, // temporary; helps diagnose PSI failures
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (N8N) { try { await fetch(N8N, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ results }) }); } catch {} }
